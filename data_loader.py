...
class DatasetFromFolder():
    def __init__(self, settings, phase_directory, depth_directory, target_directory, mask_directory, key='.png'):
        ...
        self.phase_filenames = sorted(glob.glob("/scratch/zheng.chua/RGB_dataset_noise_10/train/phase/" + '/**/*{}'.format("_phase_combined.png"), recursive=True))
        self.depth_filenames = sorted(glob.glob("/scratch/zheng.chua/RGB_dataset_noise_10/train/depth/"+ '/**/*{}'.format(self.key), recursive=True))
        self.target_filenames =sorted(glob.glob("/scratch/zheng.chua/RGB_dataset_noise_10/train/target/" + '/**/*{}'.format("_target.png"), recursive=True))
        self.mask_filenames = sorted(glob.glob("/scratch/zheng.chua/RGB_dataset_noise_10/train/mask/" + '/**/*{}'.format(self.key), recursive=True))
    ...
    def depth_calculation(self, image_depth):
        distances = torch.linspace(-self.volume_depth / 2., self.volume_depth / 2., self.number_of_planes)+ self.location_offset
    ...
    def __getitem__(self, index):
        ...
        return phase_image, distance_map.unsqueeze(0), target_image, mask_image.unsqueeze(0), os.path.basename(self.target_filenames[index])[0:6]
